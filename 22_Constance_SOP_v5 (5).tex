...
\fancyhead[R]{\small\sffamily\textcolor{textgray}{SOP \textbullet{} v4.0}}
...
% ============================================================================
% COVER PAGE — Redesigned with geometric TikZ elements
% ============================================================================
\thispagestyle{empty}
\begin{tikzpicture}[remember picture, overlay]
  % Navy header block
  \fill[navy] (current page.north west) rectangle ([yshift=-7cm]current page.north east);
  % Gold accent stripe
  \fill[gold] ([yshift=-7cm]current page.north west) rectangle ([yshift=-7.15cm]current page.north east);
  % Geometric corner triangles
  \fill[gold!30!navy] (current page.north east) -- ([yshift=-3.5cm]current page.north east) -- ([xshift=-5cm]current page.north east) -- cycle;
  \fill[steel!50!navy] (current page.north west) -- ([yshift=-2cm]current page.north west) -- ([xshift=3cm]current page.north west) -- cycle;
  % Diagonal accent lines
  \draw[gold, line width=0.6pt, opacity=0.4] ([xshift=4cm,yshift=-1cm]current page.north west) -- ([xshift=8cm,yshift=-7cm]current page.north west);
  \draw[gold, line width=0.4pt, opacity=0.3] ([xshift=5cm,yshift=-0.5cm]current page.north west) -- ([xshift=9cm,yshift=-6.5cm]current page.north west);
  \draw[gold, line width=0.4pt, opacity=0.3] ([xshift=10cm,yshift=-0.5cm]current page.north west) -- ([xshift=14cm,yshift=-6.5cm]current page.north west);
  % Title text
  \node[anchor=south west, text=white] at ([xshift=2.2cm,yshift=-4cm]current page.north west)
    {\sffamily\fontsize{38}{44}\selectfont\bfseries 22 Constance Street};
  \node[anchor=south west, text=gold] at ([xshift=2.2cm,yshift=-5.3cm]current page.north west)
    {\sffamily\fontsize{38}{44}\selectfont\bfseries Unit Trust};
  \node[anchor=south west, text=sky!80] at ([xshift=2.2cm,yshift=-6.3cm]current page.north west)
    {\sffamily\fontsize{12}{14}\selectfont Standard Operating Procedures};
  % Subtitle area
  \node[anchor=north west, text=textgray] at ([xshift=2.2cm,yshift=-8.5cm]current page.north west) {%
    \begin{minipage}{12cm}\sffamily
    \fontsize{11}{14}\selectfont
    Monthly Reporting Pack\enspace\textcolor{gold}{\textbullet}\enspace
    Fund Accounting\enspace\textcolor{gold}{\textbullet}\enspace
    Compliance\\[14pt]
    \fontsize{9.5}{13}\selectfont\color{textgray}
    \begin{tabular}{@{}r@{\enspace}l@{}}
    \textcolor{steel}{Prepared for} & \textcolor{navy}{\bfseries Centennial Property Group (CPG)} \\[3pt]
    \textcolor{steel}{Prepared by} & \textcolor{navy}{\bfseries UFS Fund Accounting} \\[3pt]
    \textcolor{steel}{Version} & \textcolor{navy}{\bfseries 4.0\enspace\textbullet\enspace March 2026} \\[3pt]
    \textcolor{steel}{Classification} & \textcolor{critleft}{\bfseries Confidential \& Internal} \\
    \end{tabular}
    \end{minipage}};
  % Bottom decorative bar
  \fill[navy] ([yshift=2cm]current page.south west) rectangle ([yshift=2.15cm]current page.south east);
  \fill[gold] ([yshift=1.85cm]current page.south west) rectangle ([yshift=2cm]current page.south east);
  \node[text=textgray, font=\sffamily\footnotesize] at ([yshift=1cm]current page.south) {%
    \textcolor{gold}{\rule{3cm}{0.3pt}}\enspace\textcolor{steel}{$\blacklozenge$}\enspace\textcolor{gold}{\rule{3cm}{0.3pt}}};
\end{tikzpicture}
\vfill
\clearpage
\tableofcontents
...
\begin{tabular}{@{}p{3.2cm} >{\centering\arraybackslash}p{1.0cm} p{9.0cm}@{}}
\toprule
...
\subsubsection{2c \& 2d --- Outgoings Receivable vs.\ Expense; Payment on Behalf}
\label{sec:grp2c}
\label{sec:grp2d}

The unifying risk across both ongoing operations and the disposal period is the same: a cost paid by the trust on behalf of another party --- a tenant or the purchaser --- must be recorded as a \textbf{receivable}, not as an expense. Misclassifying a recoverable payment as an outright expense understates assets and overstates costs. The two subsections below address each context.

\paragraph{Ongoing Operations --- Outgoings Receivable vs.\ Expense}

When the trust pays a property outgoing that is recoverable from a tenant, the trust is acting as a payment intermediary. The payment creates both a gross expense (what the trust paid) and a receivable or income (what the tenant owes or has already advanced). The primary risk is treating the entire payment as the trust's own expense and recording nothing on the receivable or income side.

{\small\sffamily
\begin{longtable}{@{} L{2.8cm} L{2.5cm} L{7.4cm} @{}}
\rowcolor{navy}
\textcolor{white}{\bfseries Stage} & \textcolor{white}{\bfseries Treatment} & \textcolor{white}{\bfseries Guidance \& Trap} \\
\midrule
\endfirsthead
\rowcolor{navy}\textcolor{white}{\bfseries Stage} & \textcolor{white}{\bfseries Treatment} & \textcolor{white}{\bfseries Guidance \& Trap} \\
\midrule
\endhead
\textbf{Outgoing incurred} & Gross expense in P\&L & Record the full cost as a property expense when incurred. Recoverable outgoings for this fund: council rates, insurance, common area electricity, water. \textbf{Trap:} do not reduce the gross expense by any anticipated recovery --- the gross expense and the recovery are separate P\&L lines. \\
\rowcolor{warmgray}
\textbf{Recovery billed to tenant} & Gross income in P\&L --- separate line; or outgoings receivable on balance sheet if not yet paid & The recovery is a separate income line, not a credit to the expense account. If the recovery has been billed but cash not yet received, hold it as an outgoings receivable on the balance sheet. \textbf{Trap:} do not net the recovery against the expense; that suppresses both sides of the P\&L and misrepresents the trust's outgoings activity. \\
\textbf{Advance received before period} & Balance sheet liability --- not yet income & Quarterly advances received before the period to which they relate are a liability, not income on receipt. \textbf{Trap:} do not recognise the advance as income on receipt and also accrue the same outgoing as an expense in the same period --- that doubles both sides of the transaction. \\
\rowcolor{warmgray}
\textbf{Annual washup} & Adjustment only & Compare total advances billed against actual costs. Record only the shortfall or credit. \textbf{Trap:} do not re-record the full year's outgoings and recoveries at washup --- those are already in the P\&L from each period. \\
\textbf{Non-recoverable items} & Expense only --- no receivable & Management fee, ASIC fee, disposal costs, and performance fee are borne by the trust alone. Do not include them in outgoings recovery calculations or in any OGS billing. No receivable arises. \\
\bottomrule
\end{longtable}}

\begin{note}[Sony Outgoings --- This Fund]
Sony (336~sqm, holdover) pays an estimated quarterly advance. Each period: (1)~confirm the gross expense is in P\&L; (2)~confirm any advance is held as a balance sheet liability, not recognised as income on receipt; (3)~confirm the recovery has been billed and recorded as a separate income line or outgoings receivable; (4)~assess whether the annual washup is due; (5)~if a CPI review date has been reached, apply the escalation per \S\ref{sec:grp4a}. Confirm 5Point and Popgun recovery entitlement under each lease schedule.
\end{note}

\paragraph{Disposal / Post-Option Exercise --- Payment on Behalf of Purchaser}

Once the Call Option is exercised, costs continue to be incurred by both the vendor trust and the purchaser. The same payment-on-behalf logic applies: if the trust pays a cost that is contractually the purchaser's obligation, the payment is a balance sheet receivable from the purchaser --- not a trust expense. The additional disposal-specific risks are deferring sale costs and then also expensing them, or double-counting settlement adjustments.

{\small\sffamily
\begin{longtable}{@{} L{3.0cm} L{2.2cm} L{7.5cm} @{}}
\rowcolor{navy}
\textcolor{white}{\bfseries Type of Cost} & \textcolor{white}{\bfseries Who Bears It} & \textcolor{white}{\bfseries Guidance \& Trap} \\
\midrule
\endfirsthead
\rowcolor{navy}\textcolor{white}{\bfseries Type of Cost} & \textcolor{white}{\bfseries Who Bears It} & \textcolor{white}{\bfseries Guidance \& Trap} \\
\midrule
\endhead
\textbf{Vendor holding costs} & Vendor --- P\&L expense & Insurance, rates, utilities, and management fees incurred while the trust holds the property remain P\&L expenses. \textbf{Trap:} do not defer these as sale costs and also record them as an expense --- that charges P\&L twice. \\
\rowcolor{warmgray}
\textbf{Deferred sale costs} & Vendor --- balance sheet until settlement & Costs exclusively connected to achieving the sale (agent commission, vendor legal, due diligence, marketing) are deferred on the balance sheet and offset against the disposal gain at settlement. Current balance: \$133,732. \textbf{Trap:} do not defer a cost on the balance sheet and also record it as a P\&L expense in the same period. \\
\textbf{Cost paid on behalf of purchaser} & Purchaser --- balance sheet receivable & If the trust pays a cost contractually borne by the purchaser, record a balance sheet receivable, not an expense. \textbf{Trap:} do not expense a payment the purchaser must reimburse. Confirm the contractual allocation with the vendor solicitor before posting. \\
\rowcolor{warmgray}
\textbf{Settlement adjustments} & Apportioned at settlement & Rates, land tax, water, and rent are adjusted as at the settlement date. \textbf{Trap:} do not expense the full annual charge and separately record a purchaser recovery --- or accrue the full annual charge and then also reduce proceeds by the same amount. Apply one consistent treatment. \\
\textbf{Post-settlement vendor obligations} & Vendor --- provision only & Warranties, make-good, and indemnities that survive settlement are the trust's obligations. Raise a provision when the obligation is probable and reliably estimable. \textbf{Trap:} do not expense an estimated obligation and also raise a provision for the same amount --- the provision entry is the expense. Release the provision as income when the obligation is discharged. \\
\bottomrule
\end{longtable}}
...
{\sffamily\footnotesize\textcolor{textgray}{22 Constance St Unit Trust \textbullet{} SOP v5.0 \textbullet{} March 2026 \textbullet{} Confidential \& Internal}}\\[0.4em]
