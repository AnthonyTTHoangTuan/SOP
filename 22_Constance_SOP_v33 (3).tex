...
%% D4 -> SC (down into Coordinator lane)
\draw[ar] (D4.south) -- ++(0,-0.7) |- (SC.east);

%% SC -> RC
\draw[ar] (SC.east) -- node[lb,above]{check} (RC.west);

%% RC Yes -> FCN (right: has notes)
\draw[ar] (RC.east) -- node[lb,above]{Yes} (FCN.west);

%% FCN -> loop back to S4 (up then left — stays above Coordinator lane top edge)
\draw[ar] (FCN.north) -- ++(0,0.8) -- ++(-11,0) |- (S4.west);

%% RC No -> S5 (down into CPG lane: approved, send to CPG)
\draw[ar] (RC.south) -- ++(0,-0.7) |- (S5.north east);
...
%% F5 -> back to S5 (loop below)
\draw[ar] (F5.south) -- ++(0,-0.5) -| (S5.south);

%% R5 No -> S6 (up into Preparer lane — vertical jump crossing lanes, direct)
\draw[ar] (R5.north) -- ++(0,5.3) -- (S6.south);

%% S6 -> D6
\draw[ar] (S6.east) -- node[lb,above]{import} (D6.west);

%% D6 -> R6 (down into Coordinator zone)
\draw[ar] (D6.south) -- ++(0,-0.6) -- (R6.north);

%% R6 Yes -> F6 (left: errors)
\draw[ar] (R6.west) -- node[lb,above]{Yes} (F6.east);

%% F6 -> back to S6 (up then right — stays within Preparer lane return)
\draw[ar] (F6.north) -- ++(0,0.7) -- (S6.south);

%% R6 No -> OB (down: proceed to page 3)
...
%% D8 -> R9a (go down into Reviewer lane boundary area)
\draw[ar] (D8.east) -- node[lb,above]{review} (R9a.west);

%% R9a Yes -> CLR (right, back in Preparer lane zone — cross up)
\draw[ar] (R9a.north) -- ++(0,0.8) -- node[lb,above]{Yes} (CLR.south|-R9a.north) -- (CLR.south);

%% CLR -> S8 (left along top of Preparer lane)
\draw[ar] (CLR.west) -- ++(- 14,0) |- (S8.north);

%% R9a No -> S9 (down into Reviewer lane)
...
%% Link S9 area to R9a (S9 feeds review gate — sequential from entry)
\draw[ar] (S8.south) -- ++(0,-0.5) |- (S9.west);

%% R9b Fail -> RET (right)
\draw[ar] (R9b.east) -- node[lb,above]{Fail} (RET.west);

%% RET -> back to S8 (go up above Reviewer lane, then left)
\draw[ar] (RET.north) -- ++(0,4.0) -- ++(-17,0) |- (S8.north);

%% R9b Pass -> S10 (down into CPG lane area — S10 is Reviewer but at boundary)
\draw[ar] (R9b.south) -- ++(0,-0.5) |- (S10.east);

%% S10 -> D10
\draw[ar] (S10.east) -- node[lb,above]{send} (D10.west);

%% D10 -> R10 (right within CPG lane)
\draw[ar] (D10.east) -- node[lb,above]{review} (R10.west);

%% R10 Yes -> RESUB
\draw[ar] (R10.east) -- node[lb,above]{Yes} (RESUB.west);

%% RESUB -> back to S10 (loop below)
\draw[ar] (RESUB.south) -- ++(0,-0.6) -| (S10.south);

%% R10 No -> ARC
\draw[ar] (R10.south) -- ++(0,-1.0) -| (ARC.north);
