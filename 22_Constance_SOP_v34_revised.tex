...
\draw[ar] (RC.south) -- ++(0,-0.8) -| (S5.north);
...
%% OD -> S8 (enter left-centre of S8)
\draw[ar] (OD.east) -- (S8.west);

%% S8 -> D8
\draw[ar] (S8.east) -- node[lb,above]{submit} (D8.west);

%% D8 -> R9a (go right then down into top of diamond)
\draw[ar] (D8.east) -- ++(1.2,0) |- (R9a.north);

%% R9a Yes -> CLR (go right; CLR is directly right at same y=-3; route up then across)
\draw[ar] (R9a.east) -- node[lb,above]{Yes} ++(0.5,0) -- ++(0,2.5) -| (CLR.north);

%% CLR -> S8 (loop back: go left above preparer lane, enter S8 from top)
\draw[ar] (CLR.west) -- ++(-15.0,0) |- (S8.north);

%% R9a No -> R9b (clean vertical drop)
\draw[ar] (R9a.south) -- node[lb,right]{No} (R9b.north);

%% S8 -> S9 (drop straight down from S8 bottom-centre into S9 top-centre)
\draw[ar] (S8.south) -- ++(0,-0.6) -- (S9.north);

%% S9 -> D9
\draw[ar] (S9.east) -- node[lb,above]{sign off} (D9.west);

%% D9 -> R9b (enter west side of diamond)
\draw[ar] (D9.east) -- node[lb,above]{review} (R9b.west);

%% R9b Fail -> RET (enter left of RET)
\draw[ar] (R9b.east) -- node[lb,above]{Fail} (RET.west);

%% RET -> S8 (go up from RET top, cross into preparer lane, enter S8 north-east corner)
\draw[ar] (RET.north) -- ++(0,5.2) -- ++(-17.0,0) |- (S8.north);

%% R9b Pass -> S10 (drop from bottom of diamond, cross lane boundary, enter S10 top-centre)
\draw[ar] (R9b.south) -- ++(0,-1.0) -- ++(-12.0,0) |- (S10.north);
...
%% R10 Yes -> RESUB (enter left of RESUB)
\draw[ar] (R10.east) -- node[lb,above]{Yes} (RESUB.west);

%% RESUB -> S10 (drop from RESUB south, go left, enter S10 south/bottom)
\draw[ar] (RESUB.south) -- ++(0,-1.2) -- ++(-17.0,0) |- (S10.south);

%% R10 No -> ARC (drop from bottom of diamond, go left and down to ARC top)
