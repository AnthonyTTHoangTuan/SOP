...
\newtcolorbox{importantnote}[1][]{
    enhanced, breakable, colback=warnamber!8!white, colframe=warnamber!75!black,
    boxrule=0pt, leftrule=5pt, borderline north={1pt}{0pt}{warnamber!50!white},
    drop shadow southeast,
    fonttitle=\sffamily\bfseries\small\color{white},
    title={$\bigstar$\enspace IMPORTANT\ifx&#1&\else\textcolor{white!70!warnamber}{\enspace\textbf{·}}\enspace #1\fi},
    attach boxed title to top left={xshift=8pt, yshift=-\tcboxedtitleheight/2},
    boxed title style={colback=warnamber!75!black, colframe=warnamber!75!black, rounded corners=all, left=6pt, right=6pt, top=2pt, bottom=2pt},
    top=14pt, bottom=6pt, left=8pt, right=8pt, before skip=22pt, after skip=8pt, rounded corners=south}
...
\begin{importantnote}[Backdated Journals --- Prior Period Trial Balance Check]
Some journals are recorded in Yardi backdated into a previous period --- sometimes many periods prior. When performing the bank reconciliation, \textbf{always reconcile and recheck the prior closing Trial Balance} in the UFS reporting pack against the current opening Trial Balance exported from Yardi. If a bank transaction cannot be located in the previous, current, or next period in Yardi, \textbf{check the Trial Balance for backdated postings before concluding the item is unmatched}. It is common for backdating to extend further back than initially expected, and the mismatch will only be visible as a TB movement rather than a direct ledger entry in the period under review. Do not raise a Sundry Debtor entry for a backdated item that has already been posted to a prior period --- confirm the TB movement first.
\end{importantnote}

\begin{importantnote}[Journals Without Supporting Invoice in Yardi --- CPG / Intercompany Charges]
It is very common for journals to be recorded in Yardi \textbf{without a corresponding invoice uploaded} in the system. This occurs most frequently for charges issued by CPG or other intercompany entities, in particular: \textbf{property accounting fees} and \textbf{management fees}. When such entries are identified during the reconciliation, \textbf{always request the supporting invoice from CPG} before the reporting pack is finalised. Do not accept a journal entry for an intercompany fee as complete until the invoice is attached to the transaction in Yardi. If CPG cannot provide the invoice within the reporting cycle, note the outstanding item in the reporting pack review comments and escalate to the second reviewer.
\end{importantnote}
...
\rowcolor{warmgray}
\textbf{Performance fee (where GAV is a threshold input)} & Hurdle rate or waterfall applied to GAV or exit proceeds & See \S\ref{sec:grp4d} below. A change in property value alters the exit proceeds estimate and therefore the performance fee calculation. Recalculate the full performance fee estimate and post the incremental difference. \\
\bottomrule
\end{longtable}}
...
Before recalculating the management fee, confirm: (1)~which input variable the agreement specifies; (2)~the applicable percentage or dollar rate; (3)~whether a CPI review date has been reached in this period (see \S\ref{sec:grp4a}); (4)~whether any amendment has been executed since the last calculation. Keep a copy of the current management agreement on the accrual workpaper tab. If there is any ambiguity, obtain written confirmation from CPG before posting.
\end{note}
