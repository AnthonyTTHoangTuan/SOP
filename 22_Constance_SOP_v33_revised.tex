...
\tikzset{
  ...
  %% ---- Arrows & labels ----
  ar/.style={-Latex, draw=textgray, line width=0.8pt, rounded corners=6pt},
...
\node[lb, right=0.15cm of OA] {\textit{Ph.1b: Bank Info}};
...
\node[dec]      (R5) at (18,-10) {All info\\received?};
...
%% ===== ARROWS (re-routed: no block crossings, split ports) =====

%% --- Ports (avoid multiple arrows into the exact same anchor) ---
\coordinate (S4_inA) at ([yshift=+6pt]S4.west);
\coordinate (S4_inB) at ([yshift=-6pt]S4.west);
\coordinate (S6_inA) at ([xshift=-6pt]S6.south);
\coordinate (S6_inB) at ([xshift=+6pt]S6.south);

%% --- Routing corridors (keep long lines off the blocks) ---
\coordinate (TOPCOR) at (0,-1.05); % just above Preparer lane content
\coordinate (RIGHTCOR) at (26.2,0); % right margin guide (x only)

%% OA -> S4
\draw[ar] (OA.east) -- (S4_inA);

%% S4 -> D4
\draw[ar] (S4.east) -- node[lb,above]{draft} (D4.west);

%% D4 -> SC (drop down at the right side, then come in)
\draw[ar] (D4.east) -- ++(1.2,0) |- (SC.east);
...
%% FCN -> back to S4 (use TOP corridor; come into a different port)
\draw[ar] (FCN.north) -- ++(0,0.9) -| (TOPCOR) -| (S4_inB);

%% RC No -> S5 (go down then left; do NOT cut across)
\draw[ar] (RC.south) -- ++(0,-0.8) -| (S5.north east);
...
%% F5 -> back to S5 (loop below CPG lane)
\draw[ar] (F5.south) -- ++(0,-0.6) -| (S5.south);

%% R5 No -> S6 (go to right margin, then straight up into S6; avoids crossing blocks)
\draw[ar] (R5.east) -- ++(6.0,0) |- (S6_inA);

%% S6 -> D6
\draw[ar] (S6.east) -- node[lb,above]{import} (D6.west);

%% D6 -> R6 (drop down cleanly)
\draw[ar] (D6.south) -- ++(0,-0.7) -- (R6.north);

%% R6 Yes -> F6
\draw[ar] (R6.west) -- node[lb,above]{Yes} (F6.east);

%% F6 -> back to S6 (return via lower port, staying inside Preparer lane)
\draw[ar] (F6.north) -- ++(0,0.8) |- (S6_inB);
...
\node[dec]  (R7b) at (24.5,-4.5) {Rec\\balances?};
...
\node[dec]  (R7d) at (19,-9.5) {Capex FV\\check OK?};
...
%% OB -> S7a (enter track 1)
\draw[ar] (OB.east) -- (S7a.west);

%% OB -> S7c (branch down to track 2: independent start)
\draw[ar] (OB.south) -- ++(0,-4.5) -- node[lb,above]{parallel} (S7c.west);

%% OB -> S7d (branch down to track 3)
\draw[ar] (OB.south) -- ++(0,-7.5) -- (S7d.west);

%% OB -> S7e (branch down to track 4)
\draw[ar] (OB.south) -- ++(0,-11.5) -- (S7e.west);

%% 7a -> D7a
\draw[ar] (S7a.east) -- node[lb,above]{verify} (D7a.west);

%% D7a -> S7b (sequential: 7b depends on 7a)
\draw[ar] (D7a.east) -- node[lb,above]{feeds} (S7b.west);

%% S7b -> D7b
\draw[ar] (S7b.east) -- node[lb,above]{post} (D7b.west);

%% D7b -> R7b (down: review rec)
\draw[ar] (D7b.south) -- node[lb,right]{review} (R7b.north);

%% R7b Yes -> F7b (left)
\draw[ar] (R7b.west) -- node[lb,above]{Yes} (F7b.east);

%% F7b -> back to S7b (up then right)
\draw[ar] (F7b.north) -- ++(0,0.7) -- (S7b.south);

%% R7b No -> OC (right, proceed to page 4)
\draw[ar] (R7b.east) -- node[lb,above]{No} (OC.west);

%% 7c -> D7c
\draw[ar] (S7c.east) -- node[lb,above]{record} (D7c.west);

%% D7c -> OC (join to off-page connector via bend below R7b)
\draw[ar] (D7c.east) -- ++(1,0) |- (OC.south);
...
%% F7d -> back to S7d
\draw[ar] (F7d.west) -- ++(-0.8,0) |- (S7d.south);
...
%% D7e -> OC (join at bottom)
\draw[ar] (D7e.east) -- ++(1,0) |- (OC.south);
...
%  Dependencies: 7f independent | 7g independent | 7g->7h | all->7i | 7i->7j
% ==========================================================================
\begin{landscape}
\vspace*{-0.4cm}
\begin{center}
...
\node[dec]  (R7g) at (19,-4.2) {Accrual\\support?};
...
\node[dec]  (SR)  at (19,-12.5) {Self-review:\\all tabs\\complete?};
...
%% OC -> S7g (branch: independent parallel start)
\draw[ar] (OC.south) -- ++(0,-2.7) -- node[lb,above]{parallel} (S7g.west);

%% 7f
\draw[ar] (S7f.east) -- node[lb,above]{amortise} (D7f.west);

%% D7f -> S7i (feeds NAV; route right then down to arrive offset left of 7h->7i)
\draw[ar] (D7f.south) -- ++(0,-0.4) -- ++(3.5,0) |- ([xshift=0.5cm]S7i.north);
...
%% F7g -> back to S7g (left then up)
\draw[ar] (F7g.west) -- ++(-0.8,0) |- (S7g.south);

%% R7g No -> route to D7h via right side (avoids F7g zone)
\draw[ar] (R7g.east) -- ++(1.2,0) |- (D7h.east);

%% 7g -> 7h sequential (left spine)
...
%% FSR -> back to S7j
\draw[ar] (FSR.west) -- ++(-0.8,0) |- (S7j.south);
...
%% ===== LAYOUT =====
%% Preparer lane          (y -1.2 to -4.5):  Step 8 submit + clear comments loop
%% 1st & 2nd Reviewer lane (y -4.5 to -9.5): Step 9 review + checklist + pass/fail gate + Step 10 send
%% CPG / Archive lane     (y -9.5 to -13.5): CPG comments gate + archive + END
...
%% Step 10 — also Reviewer responsibility (send to CPG)
...
\node[proc, minimum width=3.6cm] (ARC) at (8,-14.5) {\PN{}{Archive CPG-approved pack in UFS Shared Folder}{1st Reviewer}{BD5}};
\node[docn] (DARC) at (15,-14.5) {\DN{Final approved pack archived in UFS Shared Folder}};
\node[term, minimum width=7cm] (END) at (8,-16.3) {MONTHLY REPORTING CYCLE --- COMPLETE};
...
%% ===== ARROWS (re-routed: corridor-based, split ports, no block crossings) =====

%% --- Ports ---
\coordinate (S8_inA) at ([yshift=+6pt]S8.west);
\coordinate (S8_inB) at ([yshift=-6pt]S8.west);
\coordinate (S10_inA) at ([yshift=+6pt]S10.west);
\coordinate (S10_inB) at ([yshift=-6pt]S10.west);

%% --- Corridors ---
\coordinate (TOP5) at (0,-2.05);   % top corridor (preparer lane)
\coordinate (MID5) at (0,-4.85);   % between preparer/reviewer lanes
\coordinate (BOT5) at (0,-13.25);  % bottom corridor (CPG lane)
\coordinate (RIGHT5) at (25.2,0);  % right margin guide

%% OD -> S8
\draw[ar] (OD.east) -- (S8_inA);

%% S8 -> D8
\draw[ar] (S8.east) -- node[lb,above]{submit} (D8.west);

%% D8 -> R9a (approach from above-right to avoid brushing nodes)
\draw[ar] (D8.east) -- ++(1.0,0) |- (R9a.north);

%% R9a Yes -> CLR (go up to TOP corridor, then right; hit CLR from west)
\draw[ar] (R9a.east) -- node[lb,above]{Yes} ++(1.2,0) -| (TOP5) -| (CLR.west);

%% CLR -> back to S8 (return via TOP corridor; enter different port)
\draw[ar] (CLR.north) -- ++(0,0.8) -| (TOP5) -| (S8_inB);

%% R9a No -> R9b (clean vertical)
\draw[ar] (R9a.south) -- node[lb,right]{No} (R9b.north);

%% S8 -> S9 (entry to reviewer lane via MID corridor; avoids crossing diamonds)
\draw[ar] (S8.south) -- ++(0,-0.5) -| (MID5) -| (S9.west);
...
%% RET -> back to S8 (go up to TOP corridor then left; no block crossings)
\draw[ar] (RET.north) -- ++(0,4.2) -| (TOP5) -| (S8.north);

%% R9b Pass -> S10 (drop to boundary then come in from west port)
\draw[ar] (R9b.south) -- ++(0,-0.7) -| (S10_inA);
...
%% RESUB -> back to S10 (use BOT corridor so it never crosses blocks)
\draw[ar] (RESUB.south) -- ++(0,-0.7) -| (BOT5) -| (S10_inB);

%% R10 No -> ARC (down then left, arrive from top)
\draw[ar] (R10.south) -- ++(0,-1.2) -| (ARC.north);
