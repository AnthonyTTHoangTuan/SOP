...
\tikzset{
  ...
  %% ---- Arrows & labels ----
  ar/.style={-Latex, draw=textgray, line width=0.8pt, rounded corners=6pt, shorten >=2pt, shorten <=2pt},
...
\node[proc]  (S3) at (4,-10.0) {\PN{3}{Download Yardi TB exports; reconcile NAB TB vs bank stmt}{Preparer}{20th \& BD1}};
\node[docn]  (D3) at (11,-10.0) {\DN{cs4001, 22chts exports; Yardi vs Bank Stat Rec tab (5 cases)}};
\node[dec]   (R3) at (17,-10.0) {Rec balances\\to zero?};
...
\node[lb, right=0.15cm of OB2] {\textit{Ph.1b: Bank Info \& TB Import}};

%% ===== SWIM-LANE BACKGROUNDS =====
\begin{scope}[on background layer]
  \fill[critbg!35]  (0.5,-1.2) rectangle (25.5,-4.2);
  \fill[palesky!25] (0.5,-4.2) rectangle (25.5,-7.8);
  \fill[palesky!12] (0.5,-7.8) rectangle (25.5,-14.5);
  ...
  \node[lanelbl, fill=steel, minimum width=3cm, minimum height=0.85cm,
        rounded corners=2pt] at (-0.3,-6.0) {Coordinator};
  \node[lanelbl, fill=steel!65, minimum width=6.7cm, minimum height=0.85cm,
        rounded corners=2pt] at (-0.3,-11.2) {Preparer};
  \draw[midgray, dashed, line width=0.35pt] (0.5,-4.2)  -- (25.5,-4.2);
  \draw[midgray, dashed, line width=0.35pt] (0.5,-7.8)  -- (25.5,-7.8);
\end{scope}
...
\draw[draw=textgray, line width=0.8pt] (START.west) -- (0.5,0.5) -- (0.5,-6.0);
\draw[ar] (0.5,-2.5)  -- (S1.west);   %% branch -> S1 (CPG upload)
\draw[ar] (0.5,-6.0)  -- (S2.west);   %% branch -> S2 (Coordinator download)
...
%% D1 drops south to lane boundary, turns east to arrive at S3.north east
\draw[ar] (D1.south) -- (11,-4.2) -- (S3.north east);
%% D2 drops south to lane boundary, turns left to arrive at S3.north
\draw[ar] (D2.south) -- (11,-7.8) -- (S3.north);

%% S3 -> D3 -> R3
\draw[ar] (S3.east) -- node[lb,above]{reconcile} (D3.west);
\draw[ar] (D3.east) -- node[lb,above]{review} (R3.west);

%% R3 Yes -> OB2  (east: balanced → proceed to Phase 1b)
\draw[ar] (R3.east) -- node[lb,above]{Yes} (OB2.west);

%% R3 No -> F3  (south: differences → investigate)
\draw[ar] (R3.south) -- node[lb,right]{No} (F3.north);
...
\draw[ar] (F3.west) -- (14.5,-13.0) -- (14.5,-10.3) -- (R3.west);
...
\node[lb, left=0.1cm of OA] {\textit{from Ph.1a}};
...
\node[dec]  (R6) at (24,-4.5) {TB import\\correct?};
...
\node[lbox] (FCN) at (16.5,-6.2) {Return to Preparer\\with review notes};
...
\node[dec]      (R5) at (18,-10) {Outstanding\\items?};
\node[lbox]     (F5) at (23,-10) {Follow up with\\CPG for\\outstanding items};

%% ===== SWIM-LANE BACKGROUNDS =====
\begin{scope}[on background layer]
  \fill[palesky!20]  (0,-1.3) rectangle (26.5,-4.2);   % Preparer
  \fill[sky!30]      (0,-4.2) rectangle (26.5,-8.0);   % Coordinator
  \fill[critbg!35]   (0,-8.0) rectangle (26.5,-11.8);  % CPG
...
\draw[ar] (D4.south) -- (11.5,-4.2) -- (5.5,-4.2) -- (SC.north);
...
\draw[ar] (F5.south) -- (23,-11.5) -- (5.5,-11.5) -- (S5.south);
...
\draw[ar] (R5.north) -- node[lb,right]{No} (18,-8.2) -- (25.5,-8.2) -- (25.5,-1.6) -- (18,-1.6) -- (S6.north);
...
%% R6 No → F6  (TB incorrect — fix errors)
\draw[ar] (R6.west) -- node[lb,above]{No} (F6.east);

%% F6 → S6  (loop: fix then re-import)
\draw[ar] (F6.north) -- (S6.south);

%% R6 Yes → OB  (TB correct — proceed to Phase 2a)
\draw[ar] (R6.south) -- node[lb,right]{Yes} (OB.north);

\end{pgfonlayer}

%% FCN → S4  (on main layer so arrowhead remains visible)
%%   Route: north to lane boundary at y=-4.2, far-left to x=0.8, up to y=-3.6,
%%   east along Preparer lane to x=5.5, down into S4.south.
%%   This avoids crossing the RC→S5 vertical path.
\draw[ar] (FCN.north) -- (16.5,-4.2) -- (0.8,-4.2) -- (0.8,-3.6) -- (5.5,-3.6) -- (S4.south);
\end{tikzpicture}
}%
...
%  All sub-steps feed into 7i (NAV) on page 4.
% ==========================================================================
\begin{landscape}
\vspace*{-0.4cm}
\begin{center}
...
\node[opc] (OB) at (1,-2) {B};
\node[lb, left=0.1cm of OB] {\textit{from Ph.1b}};
...
\begin{scope}[on background layer]
  \fill[palesky!12] (0,-0.5) rectangle (27,-15);
  ...
  \draw[steel!25, dashed, line width=0.3pt] (0,-5.0)  -- (27,-5.0);
  \draw[steel!25, dashed, line width=0.3pt] (0,-8.2)  -- (27,-8.2);
  \draw[steel!25, dashed, line width=0.3pt] (0,-12.0) -- (27,-12.0);
\end{scope}
...
\draw[ar] (OB.south) -- (spC) -- node[lb,above]{parallel} (S7c.west);
\draw[ar] (spC) -- (spD) -- (S7d.west);
\draw[ar] (spD) -- (spE) -- (S7e.west);
...
\draw[ar] (D7c.east) -- (22.0,-6.5) -- (22.0,-8.5) -- (OC.north west);
...
%% F7d → S7d  (loop: west to x=3.5 corridor, north to S7d level, east into S7d)
%%   Routed via far-left corridor to avoid crossing D7d→R7d horizontal.
\draw[ar] (F7d.west) -- (3.5,-11.8) -- (3.5,-9.5) -- (S7d.south);
...
\draw[ar] (D7e.east) -- (22.0,-13.5) -- (22.0,-10.5) -- (OC.south west);
...
%  Dependencies: 7f INDEPENDENT | 7g INDEPENDENT | 7g->7h DEPENDENT |
%                7f+7h -> 7i DEPENDENT (join) | 7i->7j DEPENDENT
% ==========================================================================
\begin{landscape}
\vspace*{-0.4cm}
\begin{center}
...
%% ===== NODES (tighter vertical spacing: -1.5, -4.2, -7.0, -9.8, -12.5) =====

\node[opc] (OC) at (1,-1.5) {C};
\node[lb, left=0.1cm of OC] {\textit{from Ph.2a}};

%% 7f — independent, top track
...
%% 7g — independent, second track; 7g -> 7h
\node[proc] (S7g) at (6,-4.2) {\PN{7g}{Trust-level accruals (882.16) \& operating accruals (882.02)}{Preparer}{BD3}};
\node[docn] (D7g) at (13,-4.2) {\DN{Accruals tab; CPG invoices; contracts; prior-period patterns}};
\node[dec]  (R7g) at (19,-4.2) {Support\\missing?};
\node[lbox] (F7g) at (19,-6.2) {Obtain missing\\invoices / CPG\\confirmation};

%% 7h depends on 7g
\node[proc] (S7h) at (6,-7.0) {\PN{7h}{Inter-group loan (882.08) \& distribution payable (882.03)}{Preparer}{BD3}};
\node[docn] (D7h) at (13,-7.0) {\DN{Unitholders tab; CPG written approval required for distribution}};

%% 7i depends on ALL 7a-7h
\node[proc] (S7i) at (6,-9.8) {\PN{7i}{NAV Statement, Dashboard, ICR, unit price}{Preparer}{BD4}};
\node[docn] (D7i) at (13,-9.8) {\DN{NAV per unit, Dashboard, GL, PL, BS, CFS tabs; OE formula check}};

%% 7j depends on 7i
\node[proc] (S7j) at (6,-12.5) {\PN{7j}{Lease review, IP book value check, loan maturity flag}{Preparer}{BD4}};
\node[docn] (D7j) at (13,-12.5) {\DN{Capex tab (IP vs valuation); lease schedule; StG maturity flag}};
\node[dec]  (SR)  at (19,-12.5) {Self-review:\\gaps found?};
\node[lbox] (FSR) at (19,-14.5) {Identify gaps \&\\complete entries\\{\textit{Preparer}}};

\node[opc] (OD) at (24,-12.5) {D};
\node[lb, right=0.1cm of OD] {\textit{Ph.3: Review}};

%% ===== SWIM-LANE BACKGROUND =====
\begin{scope}[on background layer]
  \fill[palesky!12] (0,-0.3) rectangle (26,-15.5);
  \node[lanelbl, fill=steel!65, minimum width=15.2cm, minimum height=0.85cm,
        rounded corners=2pt] at (-0.7,-7.9) {\textcolor{white}{Preparer --- Step 7 Compilation (Trust-Level \& Final Outputs: 7f--7j)}};
  \draw[steel!25, dashed, line width=0.3pt] (0,-3.0)  -- (26,-3.0);
  \draw[steel!25, dashed, line width=0.3pt] (0,-8.7)  -- (26,-8.7);
  \draw[steel!25, dashed, line width=0.3pt] (0,-11.3) -- (26,-11.3);
  \draw[steel!25, dashed, line width=0.3pt] (0,-13.7) -- (26,-13.7);
\end{scope}

%% ===== ARROWS =====
%% DEPENDENCY MAP:
%%   7f: INDEPENDENT (borrowing cost — predetermined schedule)
%%   7g: INDEPENDENT of 7f (trust-level accruals — different source)
%%   7h: DEPENDENT on 7g (distribution payable needs accruals complete)
%%   7i: DEPENDENT on ALL of 7f + 7g/7h (NAV statement aggregates all entries)
%%   7j: DEPENDENT on 7i (final checks need NAV output)
%%
%% ARROW-CROSSING STRATEGY:
%%   • 7f and 7g start in parallel from OC.
%%   • D7f → S7i: left corridor x=2.5 (dedicated to 7f feed)
%%   • F7g → S7g: left corridor x=3.5 (dedicated to 7g return loop)
%%   • D7h → S7i: left corridor x=3.0 (dedicated to 7h feed, between 7f and 7g)
%%   • D7i → S7j: left corridor x=2.5 (same as 7f, now clear below y=-9.8)
%%   • FSR → S7j: left corridor x=3.5 (same as 7g loop, now clear below y=-12.5)
%%   • R7g → S7h: right corridor x=21.5 (clean, no crossings)
%%   Staggered x-values (2.5, 3.0, 3.5) keep all vertical segments visually separated.
\begin{pgfonlayer}{arrows}

%% ── Independent parallel start: OC branches to 7f and 7g ──
\draw[ar] (OC.east) -- (S7f.west);
\draw[ar] (OC.south) -- ++(0,-2.7) -- node[lb,above]{parallel} (S7g.west);

%% ── 7f: borrowing cost amortisation (INDEPENDENT) ──
\draw[ar] (S7f.east) -- node[lb,above]{amortise} (D7f.west);

%% D7f → S7i  (DEPENDENCY FEED: 7f output required for 7i)
%%   Left corridor x=2.5: D7f south, step west, drop to S7i level, east into S7i.
\draw[ar] (D7f.south) -- ++(0,-0.4) -- (2.5,-1.9) -- (2.5,-9.8) -- (S7i.west);

%% ── 7g: trust-level accruals (INDEPENDENT of 7f) ──
\draw[ar] (S7g.east) -- node[lb,above]{post}   (D7g.west);
\draw[ar] (D7g.east) -- node[lb,above]{review} (R7g.west);

%% R7g Yes → F7g  (support missing — obtain invoices)
\draw[ar] (R7g.south) -- node[lb,right]{Yes} (F7g.north);

%% F7g → S7g  (loop: left corridor x=3.5)
\draw[ar] (F7g.west) -- (3.5,-6.2) -- (3.5,-4.2) -- (S7g.west);

%% R7g No → S7h  (DEPENDENCY: 7g approved → 7h can proceed)
%%   Right corridor x=21.5: east from R7g, south to 7h level, west into S7h.
\draw[ar] (R7g.east) -- node[lb,above]{No} (21.5,-4.2) -- (21.5,-7.0) -- (S7h.east);

%% ── 7h: inter-group loan & distribution (DEPENDENT on 7g) ──
\draw[ar] (S7h.east) -- node[lb,above]{record} (D7h.west);

%% D7h → S7i  (DEPENDENCY FEED: 7h output required for 7i)
%%   Left corridor x=3.0 (staggered between 7f at x=2.5 and 7g loop at x=3.5).
\draw[ar] (D7h.south) -- ++(0,-0.4) -- (3.0,-7.4) -- (3.0,-9.8) -- (S7i.north west);

%% ── 7i: NAV Statement (DEPENDENT on ALL 7f + 7g/7h) ──
\draw[ar] (S7i.east) -- node[lb,above]{compile} (D7i.west);

%% D7i → S7j  (DEPENDENCY FEED: 7i output required for 7j)
%%   Left corridor x=2.5 (reuse of 7f channel — now clear below y=-9.8).
\draw[ar] (D7i.south) -- ++(0,-0.4) -- (2.5,-10.2) -- (2.5,-12.5) -- (S7j.west);

%% ── 7j: final checks (DEPENDENT on 7i) ──
\draw[ar] (S7j.east) -- node[lb,above]{check}  (D7j.west);
\draw[ar] (D7j.east) -- node[lb,above]{review} (SR.west);

%% SR Yes → FSR  (gaps found — identify and fix)
\draw[ar] (SR.south) -- node[lb,right]{Yes} (FSR.north);

%% FSR → S7j  (loop: left corridor x=3.5 — reuse of 7g loop channel, now clear below y=-12.5)
\draw[ar] (FSR.west) -- (3.5,-14.5) -- (3.5,-12.5) -- (S7j.north west);

%% SR No → OD  (no gaps — proceed to Phase 3)
...
\node[opc] (OD) at (1,-3) {D};
\node[lb, left=0.1cm of OD] {\textit{from Ph.2b}};
...
\node[docn]     (D9) at (13,-7) {\DN{NAV checklist, BAS checklist, Distribution checklist signed}};
\node[dec]      (R9a) at (18,-5.5) {Reviewer\\comments?};
\node[dec]      (R9b) at (18,-8.5) {All checks\\pass?};
\node[lbox]     (RET) at (23,-8.5) {Return to Preparer\\to address failed\\items};

%% --- CPG / Archive lane ---
\node[proc/rev] (S10) at (6,-11.5) {\PN{10}{Send approved NAV pack \& monthly report to CPG}{1st Reviewer}{BD5}};
...
\node[proc, minimum width=3.6cm] (ARC) at (8,-14.5) {\PN{10}{Archive CPG-approved pack in UFS Shared Folder}{1st Reviewer}{BD5}};
\node[docn] (DARC) at (15,-14.5) {\DN{Final approved pack archived in UFS Shared Folder}};
\node[term, minimum width=4cm] (END) at (8,-16.3) {END};

%% ===== SWIM-LANE BACKGROUNDS =====
\begin{scope}[on background layer]
  \fill[palesky!12] (0,-1.8) rectangle (25.5,-5.0);
  \fill[infobg!30]  (0,-5.0) rectangle (25.5,-10.0);
  \fill[critbg!20]  (0,-10.0) rectangle (25.5,-15.2);
  ...
  \node[lanelbl, fill=critred!60, minimum width=5.2cm, minimum height=0.85cm,
...
%% ===== ARROWS =====
%% DEPENDENCY CHAIN (strictly sequential):
%%   OD → S8 → D8 → S9 → D9 → R9a → [Yes: CLR → S8] [No: → R9b]
%%   R9b → [Pass: S10] [Fail: RET → S8]
...
\draw[ar] (D8.south) -- (13,-5.2) -- (6,-5.2) -- (S9.north);

%% S9 → D9  (reviewers sign off checklists)
\draw[ar] (S9.east) -- node[lb,above]{sign off} (D9.west);

%% D9 → R9a  (DEPENDENCY: signed checklists enter comment review)
%%   D9 at (13,-7), R9a at (18,-5.5). Route: east then north-east into R9a.
\draw[ar] (D9.east) -- ++(1.5,0) |- (R9a.west);

%% R9a Yes → CLR  (reviewer comments exist — preparer clears)
%%   Route: R9a east to x=21.5, north to CLR return corridor y=-1.8, west to CLR.north.
%%   Corridor y=-1.8 is dedicated to CLR return (lower of two return corridors).
\draw[ar] (R9a.east) -- node[lb,above]{Yes} (21.5,-5.5) -- (21.5,-1.8) -- (CLR.north);

%% CLR → S8  (loop: re-submit pack after clearing comments)
%%   Route: CLR north to CLR corridor y=-1.8, west to S8 x=6, south into S8.north.
\draw[ar] (CLR.north) -- (20,-1.8) -- (6,-1.8) -- (S8.north);

%% R9a No → R9b  (no comments — proceed to formal sign-off check)
\draw[ar] (R9a.south) -- node[lb,right]{No} (R9b.north);

%% R9b Fail → RET  (checks failed — return to preparer)
\draw[ar] (R9b.east) -- node[lb,above]{Fail} (RET.west);

%% RET → S8  (loop: address failures, re-submit)
%%   Route: RET north to RET corridor y=-1.2 (above CLR corridor), west to x=4.2,
%%   south into S8.north west. Separate from CLR return.
\draw[ar] (RET.north) -- (23,-1.2) -- (4.2,-1.2) -- (S8.north west);

%% R9b Pass → S10  (DEPENDENCY: all checks passed — send to CPG)
%%   Route: R9b south to y=-10.0 (lane boundary), west at y=-10.0 to x=6, south into S10.north.
%%   Stays above RESUB return corridor at y=-13.0.
\draw[ar] (R9b.south) -- node[lb,right]{Pass} (18,-10.0) -- (6,-10.0) -- (S10.north);
...
%% RESUB → S10  (loop: bottom corridor y=-13.0, west to x=3.5, north into S10.south)
\draw[ar] (RESUB.south) -- (23,-13.0) -- (3.5,-13.0) -- (S10.south);

%% R10 No → ARC  (no CPG comments — archive)
\draw[ar] (R10.south) -- node[lb,right]{No} (18,-13.5) -- (ARC.north);

%% ARC → DARC  (archive the approved pack)
\draw[ar] (ARC.east) -- node[lb,above]{archive} (DARC.west);

%% DARC → END  (archived output closes cycle)
\draw[ar] (DARC.south) -- ++(0,-0.8) -| (END.east);
