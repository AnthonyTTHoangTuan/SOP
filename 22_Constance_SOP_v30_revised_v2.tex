...
{\scriptsize\sffamily
\begin{longtable}{@{}L{3.2cm} X L{3.2cm}@{}}
...
\endfirsthead
% ── continuation header ────────────────────────────────────────────────────────
\multicolumn{3}{l}{\footnotesize\itshape Table~\ref{tab:dist-scenarios} continued --- Distribution component scenarios}\\[2pt]
\rowcolor{navy}\textcolor{white}{\bfseries Scenario} & \textcolor{white}{\bfseries Analysis} & \textcolor{white}{\bfseries Dominant Components} \\
\midrule
\endhead
% ── continuation footer ────────────────────────────────────────────────────────
...
\rowcolor{warmgray}
$D > AP^+$, $AP^+ \geq \NetTI$ (exceeds accounting profit --- true ROC) & Excess above $AP^+$ is ROC: $\CROC = D - AP^+$.  Tax deferred arises for the middle band ($AP^+ - \NetTI$) because accounting profit exceeds taxable income.  This is the typical sale-year scenario: large distribution funded by settlement proceeds, accounting profit positive but below $D$. & $\COI$, $\CCG$, $\CTD$, $\CROC$ \\
$D > AP^+$, $AP^+ < \NetTI$ (accounting profit below taxable income) & Arises when a non-deductible accounting write-down (e.g.\ AASB~140 fair-value impairment) compresses accounting profit below taxable income.  $\TDmax = 0$ (no timing band); $\CROC = D - \NetTI$ (since $AP^+ = 0$ or $AP^+ < \NetTI$ removes the middle band); there is no $\CTD$.  Verify that no timing-difference deduction is being double-counted. & $\COI$, $\CCG$, $\CROC$ (no $\CTD$) \\
\rowcolor{warmgray}
...
$\NCG > D$ (net capital gain alone exceeds distribution) & The sequential allocation constrains $\CCG$ to the lesser of remaining $D$ and $\NCG$.  If $\TOI > 0$, $\COI = \min(D, \TOI)$ is allocated first, then $\CCG = D - \COI$; the entire distribution is taxable.  If $\TOI = 0$ (prior-year losses extinguished ordinary income), $\CCG = D$.  No $\CTD$ or $\CROC$ arises because $D \leq \NCG \leq \NetTI$.  The undistributed portion of $\NetTI$ is taxed to unitholders under s97 --- they pay tax on income never received as cash. & $\COI$ (if $\TOI > 0$), then $\CCG$ (absorbs remainder; $\CCG = D$ only when $\TOI = 0$); no $\CTD$, no $\CROC$ \\
\rowcolor{warmgray}
...
Div~152 small business CGT concessions apply & The basic conditions in Subdiv~152-A are satisfied (net assets $\leq$\$6M or aggregated turnover $<$\$2M; active asset test met).  After the Div~115 discount, a further Div~152 exclusion ($\CSBC > 0$) reduces the assessable gain permanently.  For a passive property trust holding a leased commercial building this is almost always inapplicable; a positive $\CSBC$ in such a trust is a red flag requiring tax agent confirmation.  If applicable: $\CSBC = D - \COI - \CCG - \CTD - \CROC$. & $\COI$, $\CCG$, $\CTD$ (if any), $\CROC$ (if any), $\CSBC$ \\
\rowcolor{warmgray}
NANE income (non-assessable non-exempt) & Amounts excluded from assessable income by specific statutory provision (e.g.\ certain government grants, MIT cross-staple income under s12-437).  Rare for a passive property trust.  NANE is \textbf{not} part of $D$ and does \textbf{not} reduce the unitholder's cost base (s104-71(1)).  NANE falls outside the five-component formula entirely; it is disclosed as a separate line on the AMMA or distribution statement.  If present, the Preparer must confirm the relevant statutory exclusion with the tax agent. & Outside component formula --- separate AMMA disclosure only \\
\end{longtable}
}
...
\label{tab:dist-matrix}
% C_SBC column added; Row 4 label tightened; two missing rows added; Div~152 and NANE footnote added.
\begin{tabularx}{\textwidth}{@{}L{2.5cm} C{0.85cm} C{0.85cm} C{0.85cm} C{0.85cm} C{0.85cm} C{0.85cm} L{2.3cm}@{}}
\rowcolor{navy}
\textcolor{white}{\bfseries Scenario} &
\textcolor{white}{\bfseries C\textsubscript{OI}} &
\textcolor{white}{\bfseries C\textsubscript{CG}} &
\textcolor{white}{\bfseries C\textsubscript{TD}} &
\textcolor{white}{\bfseries C\textsubscript{ROC}} &
\textcolor{white}{\bfseries C\textsubscript{SBC}} &
\textcolor{white}{\bfseries C\textsubscript{disc}} &
\textcolor{white}{\bfseries Unitholder tax outcome} \\
\midrule
$D = \NetTI$ & Yes & Yes & Nil & Nil & Nil & Separate & Tax matches cash \\
\rowcolor{warmgray}
$D < \NetTI$ & Yes & Yes & Nil & Nil & Nil & Separate & Tax exceeds cash --- shortfall; unitholder funds gap personally \\
$\NetTI < D \leq AP^+$ & Yes & Yes & Yes & Nil & Nil & Separate & Deferred tax on future unit sale via lower cost base \\
\rowcolor{warmgray}
$D > AP^+ \geq \NetTI$ & Yes & Yes & Yes & Yes & Nil & Separate & Cost base reduced; gain on disposal if CB reaches zero \\
$D > AP^+$, $AP^+ < \NetTI$ & Yes & Yes & Nil & Yes & Nil & Separate & No timing band; $\CROC$ for excess over $\NetTI$; no $\CTD$ \\
\rowcolor{warmgray}
$AP < 0$, $\NetTI > 0$ & Yes & Yes & Nil & Possible & Nil & Separate & No timing difference; ROC if $D > \NetTI$ \\
$AP < 0$, $\NetTI = 0$ & Nil & Nil & Nil & Yes & Nil & Nil & Entire distribution is ROC; cost base reduced dollar-for-dollar; no current tax \\
\rowcolor{warmgray}
$\NCG > D$ & Possible & Yes & Nil & Nil & Nil & Separate & All of $D$ is taxable ($\COI$ first, then $\CCG$); s97 tax on undistributed $\NetTI$ share \\
$TL$ eliminates $\TOI$ & Nil & Yes & Possible & Possible & Nil & Separate & Capital-only distribution; carry-forward losses reduce ordinary income to nil \\
\rowcolor{warmgray}
$D = 0$ (no dist.) & Nil & Nil & Nil & Nil & Nil & Nil & s97 assessment on trust net income; no cash received; unitholder funds tax personally \\
Div~152 concessions apply$^\dagger$ & Yes & Yes & Possible & Possible & Yes & Separate & Part of gain permanently excluded; $\CSBC > 0$; red flag for passive trust --- confirm with tax agent \\
\bottomrule
\end{tabularx}
\smallskip
\raggedright
\textit{$^\dagger$\,Div~152 is almost always inapplicable for a passive property trust.  A positive $\CSBC$ is a red flag requiring tax agent sign-off.  NANE income (if any) falls outside this formula entirely and is disclosed as a separate memorandum item on the AMMA or distribution statement; NANE does not reduce the unitholder's unit cost base (s104-71(1)).}
\end{table}
